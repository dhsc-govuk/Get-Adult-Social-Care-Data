    ...
    if (careProvider?.data_point ?? 0 > region.data_point) {
      return (
        <>
          ...
          occupancy for all adult social care beds compared to the median
          occupancy of all care homes in {locations[2]} ({region.data_point}%),
    ...
    if (careProvider?.data_point ?? 0 < region.data_point) {
      return (
        <>
          ...
          homes in Suffolk {locations[2]} ({region.data_point}%), suggesting
          greater availability for new admissions.
        </>
      );
    }
    return (
      <>
        {locations[1]} has{' '}
        <strong>({careProvider?.data_point ?? 0}%) occupancy </strong> occupancy
